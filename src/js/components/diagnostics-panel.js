...
export function createDiagnosticsPanel() {
    ...
    async function updateGPUInfo() {
        ...
        try {
            ...
            if (gpuInfo.adapter) {
                gpuContent.appendChild(createMetric('Adapter', gpuInfo.adapter));
            }

            if (gpuInfo.limits) {
                const maxBufferSize = (gpuInfo.limits.maxBufferSize / 1024 / 1024).toFixed(0);
                gpuContent.appendChild(createMetric('Max Buffer', `${maxBufferSize} MB`));

                gpuContent.appendChild(
                    createMetric('Max Texture Size', `${gpuInfo.limits.maxTextureDimension2D}px`)
                );
            }
            ...
            gpuContent.appendChild(
                createMetric(
                    ...
                    perfEstimate.tier === 'high'
                        ? 'text-green-400'
                        : perfEstimate.tier === 'medium'
                          ? 'text-yellow-400'
                          : 'text-red-400'
                )
            );

            gpuContent.appendChild(createMetric('Expected Latency', perfEstimate.expectedLatency));
        } catch (error) {
            gpuContent.appendChild(createMetric('Error', error.message, 'text-red-400'));
        }
    }
    ...
    async function updateModelState() {
        ...
        if (state.isLoaded) {
            modelContent.appendChild(
                createMetric('Avg Inference', `${(state.avgInferenceTime / 1000).toFixed(2)}s`)
            );
        }
    }
    ...
    async function updatePerformance() {
        ...
        if (state.isLoaded && state.avgInferenceTime > 0) {
            ...
            perfContent.appendChild(
                createMetric('Avg Inference Time', `${avgTimeSec.toFixed(2)}s`)
            );

            // Estimate tokens/s (assuming ~50 tokens per inference)
            const tokensPerSecond = (50 / avgTimeSec).toFixed(1);
            perfContent.appendChild(createMetric('Est. Tokens/s', tokensPerSecond));

            // FPS equivalent
            const fps = (1 / avgTimeSec).toFixed(2);
            perfContent.appendChild(createMetric('Max FPS', fps + ' fps'));

            // Estimated memory (rough approximation)
            // FastVLM 0.5B ~2GB model size
            const estimatedMemory = '~2000 MB';
            perfContent.appendChild(createMetric('Est. Memory', estimatedMemory));
            ...
            perfContent.appendChild(
                createMetric('Status', 'No performance data yet', 'text-gray-400')
            );
        }
    }
