...
function createURLBadge(urlData) {
    const badge = createElement('div', {
        className:
            'inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-white/10 border border-white/20 hover:bg-white/20 transition-colors',
    });
...
function showURLConfirmationDialog(urlData) {
    ...
    const overlay = createElement('div', {
        className:
            'fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] animate-fade-in',
    });
    ...
    const urlDisplay = createElement('div', {
        className:
            'bg-black/30 rounded-lg p-4 mb-4 max-h-32 overflow-y-auto border border-white/10',
    });
    ...
    const securityNotice = createElement('p', {
        ...
        textContent:
            '⚠️ Never open links from untrusted sources. Links may lead to phishing sites or malware.',
    });
