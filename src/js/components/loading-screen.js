...
export function createLoadingScreen(onPhaseChange, onComplete, onError) {
    ...
    const subtitle = createElement('p', { className: 'ls-subtitle', text: 'This may take a moment' });
    ...
    const progressFill = createElement('div', { className: 'ls-progress-fill', style: { width: '0%' } });
    ...
    const thinkingText = createElement('span', { className: 'ls-thinking-text', text: 'Checking GPU availability...' });
    ...
    setTimeout(async () => {
        try {
            ...
            await vlmService.loadModel((message, progressPercent) => {
                ...
                else if (message.includes('Processor loaded')) thinking = 'Allocating tensor memory...';
                else if (message.includes('Loading model')) thinking = 'Loading ONNX graph...';
                else if (message.includes('Model loaded')) thinking = 'Optimizing execution graph...';
