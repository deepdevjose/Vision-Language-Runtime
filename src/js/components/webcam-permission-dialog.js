...
export function createWebcamPermissionDialog(onPermissionGranted, onPermissionDenied) {
    ...
    if (!isSecureContext) {
        ...
        warnIcon.style.background =
            'linear-gradient(135deg, rgba(245, 158, 11, 0.12), rgba(245, 158, 11, 0.03))';
        warnIcon.style.color = '#f59e0b';

        const warnTitle = createElement('h2', {
            className: 'webcam-permission-title',
            text: 'Secure Connection Required',
        });
        const warnSub = createElement('p', {
            className: 'webcam-permission-subtitle',
            text: 'HTTPS Required',
        });
        warnSub.style.color = '#f59e0b';
        const warnMsg = createElement('p', {
            className: 'webcam-permission-error',
            text: 'Camera access requires HTTPS. Use localhost or a secure connection.',
        });
        warnMsg.style.color = 'rgba(245, 158, 11, 0.8)';
        const warnLog = createElement('p', {
            className: 'webcam-permission-micro-log',
