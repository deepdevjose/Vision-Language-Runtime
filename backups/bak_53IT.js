...
export function createCaptioningView(videoElement) {
    ...
    const liveCaptionText = createElement('p', { className: 'rt-caption-live-text', text: '\u23f3 Waiting for first inference\u2026' });
    ...
    const outputText = createElement('div', { className: 'rt-output-text', text: 'Waiting for first inference\u2026' });
    ...
    function setThinking(steps) {
        ...
        outputText.innerHTML = steps.map(function(s, i) {
            var cls = i === steps.length - 1 ? ' active' : ' done';
            var prefix = i < steps.length - 1 ? '\u2713' : '\u27f3';
            return '<span class="rt-thinking-step' + cls + '">' + prefix + ' ' + s + '</span>';
        }).join('');
    }
            ...
            while (!signal.aborted && isRunning) {
                if (videoElement && videoElement.readyState >= 2 && videoElement.videoWidth > 0) {
                    ...
                    try {
                        ...
                        let streamSteps = ['Analyzing image\u2026'];

                        const result = await vlmService.runInference(
                            videoElement,
                            currentPrompt,
                            (streamedText) => {
                                if (streamedText.length > 20 && streamSteps.length < 3) {
                                    streamSteps.push('Generating response\u2026');
                                }
                                setThinking(streamSteps);
                                const stepsHtml = streamSteps.map((s, i) => {
                                    const cls = i === streamSteps.length - 1 ? ' active' : ' done';
                                    const prefix = i < streamSteps.length - 1 ? '\u2713' : '\u27f3';
                                    return '<span class="rt-thinking-step' + cls + '">' + prefix + ' ' + s + '</span>';
                                }).join('');
                                outputText.innerHTML = stepsHtml + '<span class="rt-streaming">' + streamedText + '</span>';
                        ...
                        if (result && !signal.aborted) {
                            ...
                            outputTime.textContent = elapsed + 's \u00b7 ' + new Date().toLocaleTimeString();
